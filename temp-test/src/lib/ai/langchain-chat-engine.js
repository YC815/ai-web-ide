...
export async function createLangChainChatEngine(chatHistory, projectName) {
  ...
  if (projectName) {
    try {
      ...
      logger.info(
        `[ChatEngine] Setting up Docker tools for project: ${projectName} (Normalized: ${normalizedName})`
      );
      const containerContext = getDockerContextByName(normalizedName);
      if (!containerContext) {
        logger.error(
          `[ChatEngine] Could not find a running container for project: ${projectName}. Docker tools will not be available.`
        );
      } else {
        ...
        logger.info(
          `[ChatEngine] Docker context created for container ID: ${containerContext.containerId}`
        );
        const fileSystemTool = new DockerFileSystemTool(
          dockerContext,
          projectName
        );
        ...
        logger.info(
          `[ChatEngine] Docker tools for project '${projectName}' have been successfully added.`
        );
      }
    } catch (error) {
      const errorMessage =
      ...
      logger.error(
        `[ChatEngine] Failed to initialize Docker tools for project ${projectName}: ${errorMessage}`
      );
      const dockerErrorTool = {
        ...
        func: () =>
          `Error: Could not connect to the Docker container for project ${projectName}. Please check if the project is running correctly.`,
      };
      tools.push(dockerErrorTool);
    }
  } else {
    logger.warn(
      "[ChatEngine] No project name provided. Docker-specific tools are disabled."
    );
  }
  // 創建向量存儲用於知識庫搜尋
