...
export class AIContextManager {
   ...
   * 獲取完整的專案快照 - AI的專案理解入口
   */
  async getProjectSnapshot(forceRefresh: boolean = false): Promise<AIToolResponse<ProjectSnapshot>> {
    const now = Date.now();
    ...
    try {
      ...
      // 1. 檢查專案基本狀態
      const statusResult = await this.toolkit.project.getProjectStatus();
      if (!statusResult.success) {
        return {
          success: false,
          error: `無法獲取專案狀態: ${statusResult.error}`
        };
      }

      // 2. 獲取檔案結構
      const structureResult = await this.toolkit.project.getProjectStructure();
      if (!structureResult.success) {
        return {
          success: false,
          error: `無法獲取專案結構: ${structureResult.error}`
        };
      }

      // 3. 分析關鍵檔案
      const keyFiles = await this.analyzeKeyFiles(structureResult.data || []);
      ...
      // 6. 檢查建置狀態
      const buildStatus = await this.analyzeBuildStatus();

      // 建立專案快照
      this.projectSnapshot = {
        projectInfo: {
          name: this.projectContext.projectName,
          type: keyFiles.projectType || 'unknown',
          version: keyFiles.version || '1.0.0',
          isInitialized: statusResult.data?.isInitialized || false
        },
        fileStructure: {
          directories: this.extractDirectories(structureResult.data || []),
          files: structureResult.data || [],
   ...
   * 智能建議 - 根據專案狀態給出AI操作建議
   */
  async getSmartSuggestions(): Promise<AIToolResponse<string[]>> {
